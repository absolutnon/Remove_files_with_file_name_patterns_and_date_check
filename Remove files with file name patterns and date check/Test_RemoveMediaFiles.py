...
def remove_media(file_pattern, path_to_file):
    ...
    file_total = "Total file will be deleted: " + str(counter) + " files"
...
def log_deleted_files(file_removed):
    ...
    with open(r'D:/TEST_Removed_files_' + datetime.datetime.now().strftime("%Y_%m_%d-%I%M%S_%p") + '.txt', 'a', newline='') as file_rm:
